...
export const callback = (req, res) => {
  ...
  if (state === null || state !== storedState) {
    ...
    axios.post('https://accounts.spotify.com/api/token', queryString.stringify({
      ...
      headers: {
        Authorization: 'Basic ' + (Buffer.from(spotify.clientId + ':' + secret.spotifyClientSecret).toString('base64'))
      }
    }).then((response) => {
      if (response.status === 200) {
        ...
        res.redirect(`/spotify-auth.html#${queryString.stringify({ access_token: accessToken, refresh_token: refreshToken })}`)
